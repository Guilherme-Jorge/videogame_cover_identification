...
) -> tuple[str, str, int, int, float, int, int, str, str, str, int, int]:
    """Set up and validate training parameters."""
...
def _setup_model(device: str, dim: int) -> ImgModel:
    """Set up the CLIP model for fine-tuning."""
    import open_clip
...
def _setup_data_and_optimizer(
    ...
    epochs: int,
    net: ImgModel,
) -> tuple[DataLoader, torch.optim.Optimizer, torch.optim.lr_scheduler.LRScheduler]:
    """Set up dataset, dataloader, optimizer and scheduler."""
...
def _setup_amp(amp: str) -> tuple[str, torch.dtype | None, bool, torch.amp.GradScaler]:
    ...
    return device_type, amp_dtype, use_scaler, scaler
...
def train_finetune(
    ...
    net = _setup_model(device, dim)
    dl, opt, sched = _setup_data_and_optimizer(
        jsonl_path, root_dir, batch_size, workers, lr, grad_accumulation_steps, epochs, net
    )

    # Loss function
    loss_fn = NTXent(temperature=config.training.temperature)

    device_type, amp_dtype, use_scaler, scaler = _setup_amp(amp)
...
def main():
    ...
    ap.add_argument(
        "--jsonl-path", default=config.training.jsonl_path, help="Path to metadata.jsonl"
    )
    ap.add_argument("--root-dir", default=None, help="Root dir for image files")
    ap.add_argument("--epochs", type=int, default=config.training.epochs, help="Number of epochs")
    ap.add_argument("--batch-size", type=int, default=config.training.batch_size, help="Batch size")
    ap.add_argument("--lr", type=float, default=config.training.lr, help="Learning rate")
    ap.add_argument(
        "--workers", type=int, default=config.training.workers, help="Number of workers"
    )
    ap.add_argument(
        "--dim", type=int, default=config.training.dim, help="Output embedding dimension"
    )
    ap.add_argument(
        "--amp",
        choices=["none", "fp16", "bf16"],
        default=config.training.amp,
        help="Automatic mixed precision",
    )
    ap.add_argument("--out-path", default=config.training.out_path, help="Output path for model")
    ap.add_argument("--device", default=config.device, help="Device for training")
    ap.add_argument(
